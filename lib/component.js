...
Component.prototype.build = function(instr, options, sys, cb) {
  ...
  var ipath = path.join(this.path, 'directives', instr || 'default') + '.js';
  try {
    var ifn = require(ipath)(options);
    if (typeof ifn == 'function') {
      var ctx = new InstructionContext(this.name);
      ctx.init(sys);
...
function InstructionContext(cname) {
  this.cname = cname;
  this.list = [];
}
...
InstructionContext.prototype.init = function(sys) {
  ...
  for (var i = 0, len = facs.length; i < len; i++) {
    ...
    this[type] = resourceDecl(type, fac).bind(this);
  }
  
  this.template = templateFn();
}

function resourceDecl(type, facility) {
  
  return function(name, opts) {
    var def = { __type__: type };
    if (facility.compile) {
      var copts = facility.compile(name, opts);
...
function templateFn() {
  ...
  return function(name, opts) {
    var template = this.cname + '/' + name;
    if (utils.isOverride(name)) {
      template = name;
    }
  
    var locals = {};
    // TODO: Merge system attributes into locals.
    utils.merge(locals, opts);
  
    return { _fn: 'render', template: template, locals: locals }
  }
}
