...
Blueprint.prototype.fabricate = function(fn, done) {
  if (typeof fn === 'function') {
    this._fabricators.push(fn);
    return this;
  }
  
  // private implementation that traverses the chain of fabricators, fabricating
  // the intermediate resource definition
  var self = this
    , def = fn;
  
  var stack = this._fabricators;
  (function iter(i, err, fdef) {
    // an error or fabricated definition was obtained, done
    if (err || fdef) { return done(err, fdef); }
  
    var layer = stack[i];
    if (!layer) { return done(null, def); }
    
    try {
      layer(self, def, function(e, fd) { iter(i + 1, e, fd); } )
    } catch(e) {
      return done(e);
    }
  })(0);
...
Blueprint.prototype.boot = function(file, env, options, callback) {
  ...
  this.fabricate(require('./fabricators/file')());
