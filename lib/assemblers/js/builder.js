...
function JSBuilder(file, options) {
  this.file = file;
  this.options = options;
}

JSBuilder.prototype.build = function(sys, dtive, comp, bp, cb) {
  try {
    var fn = require(this.file)(this.options);
    if (typeof fn == 'function') {
      var ctx = new Context(comp.name);
      ctx.init(sys);
      ...
      return cb(new Error("Unable to invoke directive '" + comp.name + "/" + dtive + "'"));
    }
  } catch (e) {
    return cb(e);
  }
